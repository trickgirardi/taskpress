        ...
        <div className='flex flex-col items-center justify-around h-full max-w-xs bg-zinc-800 rounded-md p-3'>
            ...
            <ul className='m-3 p-3 w-full h-full border border-zinc-500 bg-zinc-900 rounded-md'>
                {taskList.map((task, index) => (
                    <li onClick={() => toggleTaskCompletion(index)}  style={{ textDecoration: task.completed ? 'line-through' : 'none' }} className='hover:cursor-pointer text-white border-b border-b-zinc-500 mt-2 mb-2' key={index}>{task.name}</li>
                ))}
            </ul>
