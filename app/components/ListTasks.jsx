...
export default function AddTask() {
    ...
    return (
        <div className='flex flex-col items-center justify-around h-full w-screen max-w-xl bg-zinc-800 rounded-md p-3'>
                ...
                {taskList.map((task, index) => (
                    <li className='flex flex-row justify-between items-center text-white border-b border-b-zinc-500 mt-2 mb-2' key={index}>
                        ...
                        <div className=' flex flex-row gap-2'>
                            <button onClick={() => startEditTask(index)} className='text-zinc-500 text-center'>edit</button>
            ...
            <button className={`m-3 mt-0 rounded text-center p-3 text-slate-200 ${editIndex !== null ? 'bg-yellow-600' : 'bg-slate-700'}`} onClick={handleAddTask}>{editIndex !== null ? 'Save Changes' : 'Create Task'}</button>
        </div>
    )
}